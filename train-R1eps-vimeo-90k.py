...
def cal_W1(ssf, encoder, decoder, decoder_hat, discriminator_JD, discriminator_AR, discriminator_FMD, test_loader, list_models, FMD, JD, AR):
    ...
    for i, x in enumerate(iter(test_loader)):
        with torch.no_grad():
            ...
            if AR:
                ...
                new_metric_real_validity = discriminator_AR(real_vid_new)
                new_metric_fake_validity = discriminator_AR(fake_vid)
            ...
            if AR:
                AR_distance.append(torch.sum(new_metric_real_validity) - torch.sum(new_metric_fake_validity))
            if FMD:
...
def main():
    ...
    # we check what is metric used for training (FMD,JD,ar,MSE)
    ...
    FMD = JD = AR = (lambda_FMD == 0 and lambda_JD == 0 and lambda_AR == 0) 
    ...
    + f'lambdaFMD_{lambda_FMD}|lambdaaAR_{lambda_AR}|lambdaMSE_{lambda_MSE}'
    print ("Settings: ", folder_name)

    os.makedirs('./saved_models/vimeo-90k/'+ folder_name, exist_ok=True)
    ...
    discriminator_AR = Discriminator_v3(out_ch=2).to(device) if AR else None
    ...
    list_models = [discriminator_JD, discriminator_AR, discriminator_FMD, ssf] 
    ...
    list_opt = [opt_ssf, opt_JD, opt_AR, opt_FMD] 
    ...
    for epoch in range(total_epochs):
        ...
        for i,x in enumerate(iter(train_loader)):
            ...
            if AR:
                fake_vid = torch.cat((x_1_hat, x_hat), dim = 1)
                real_vid_AR = torch.cat((x_1_hat, x_cur), dim = 1).detach()
                AR_fake_validity = discriminator_AR(fake_vid)
                AR_real_validity = discriminator_AR(real_vid_AR)
                gradient_penalty_AR = compute_gradient_penalty(discriminator_AR, real_vid_AR.data, fake_vid.data)
                errAR =  -torch.mean(AR_real_validity) + torch.mean(AR_fake_validity) + lambda_gp * gradient_penalty_AR
                errAR.backward()
                opt_AR.step()
            ...
            if i%skip_fq == 0:
                ...
                if AR:
                    fake_vid = torch.cat((x_1_hat, x_hat), dim = 1)
                    fake_validity_AR = discriminator_AR(fake_vid)
                ...
                loss = lambda_MSE*mse(x_hat, x_cur) + lambda_JD*errJD_ + lambda_AR*errAR_ + lambda_FMD*errFMD_
                loss.backward()
                opt_ssf.step()
        ...
        if ((epoch+1) % 10 == 0) or epoch == 0:
            f = open('./saved_models/'+ folder_name + "/performance.txt", "a")
            JD_loss, MSE_loss, AR_loss, FMD_loss = cal_W1(ssf, encoder, decoder, decoder_hat, discriminator_JD, discriminator_AR, discriminator_FMD, test_loader, list_models, FMD, JD, AR)
