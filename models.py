...
class Discriminator_v3_vimeo(nn.Module):
    def __init__(self, ch=64, out_ch=2):
        super(Discriminator_v3_vimeo, self).__init__()

        self.net = nn.Sequential(
            spectralnorm(nn.Conv3d(out_ch, ch, 4, 2, 1)),
            nn.LeakyReLU(0.2, inplace=True),

            spectralnorm(nn.Conv3d(ch, ch*2, 4, 2, 1)),
            nn.LeakyReLU(0.2, inplace=True),

            spectralnorm(nn.Conv3d(ch*2, ch*4, 4, 2, 1)),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv3d(ch*4, ch*8, 4, 2, 1),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv3d(ch*8, 1, 4, 1, 0),
        )
...
class Discriminator_uvg(nn.Module):
